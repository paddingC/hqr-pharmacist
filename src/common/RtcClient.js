...
export default class RtcClient {
    constructor(options) {
        ...
        this.liuState = {
            roomid: options.record.roomid,
            userid: options.record.userid2,
            companyid: options.record.companyid,
        }
        /*this.leaveRoom = {
            mduserid: options.record.userid1,
            companyid: options.record.companyid,
        }*/


        console.log(store.creadState);
    ...
    async join() {
        ...
        try {
            ...
            store.commit('user/SET_SGEBEI', true)
            ...
            store.commit('app/SET_CALLFLASE', false)
            store.commit('user/SET_SGEBEI', false)
            router.push('/home/accountOverview/information')
            alert ('请检查您的设备是否正常');
            //this.leave()
            return
            // try {
    ...
    async switchCameraClick () {
        let camindex = -1
        TRTC.getCameras().then(devices => {
            console.log(devices);
            console.log('摄像头设备',this.cameraId);
            devices.forEach((dev,index) => {
                if (this.cameraId == dev.deviceId){
                    console.log(this.cameraId);
                    camindex = (index+1)%devices.length;
                }
            })
            if (devices.length !== 0) {
                this.localStream_.switchDevice('video', devices[camindex].deviceId).then(() => {
                    this.cameraId = devices[camindex].deviceId
                    console.log("成功切换")
                }).catch(err => {
                    console.log(err, "切换失败")
                })
    ...
    handleEvents() {
        ...
        this.client_.on('stream-subscribed', evt => {
            ...
            console.log('远端流订阅成功');
            if (sessionStorage.getItem('noVideo') === '2') {
                timer = setInterval(()=>{
                    if (count >= 5){
                        clearInterval(timer)
                        return false
                    ...
                    axios({
                        method: 'POST',
                        url: 'https://yunservice.inca.com.cn/dzsf/tencentcloud/queryStreamState',
